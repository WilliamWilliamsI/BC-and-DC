...
public class TxHandler {
    ...
    /**
     * Creates a public ledger whose current main.java.UTXOPool (collection of unspent transaction outputs) is
     * {@code utxoPool}. This should make a copy of utxoPool by using the main.java.UTXOPool(main.java.UTXOPool uPool)
     * constructor.
     */
    ...
    /**
     * @return true if:
     * (1) all outputs claimed by {@code tx} are in the current main.java.UTXO pool,
     * (2) the signatures on each input of {@code tx} are valid,
     * (3) no main.java.UTXO is claimed multiple times by {@code tx},
    ...
    public boolean isValidTx(Transaction tx) {
        ...
        for (int i = 0; i < tx.getInputs().size(); i++) {
            ...
            //verify if output claimed by tx are in the current main.java.UTXO pool
            ...
            //verify main.java.UTXO is claimed multiple times by tx

            if (index_used.contains(u)) {
                System.out.println("multiple use of same main.java.UTXO");
                return false;
            } else {
                index_used.add(u);
            }
     ...
     * updating the current main.java.UTXO pool as appropriate.
     */
    ...
    /**
     * handle a single transaction and update the main.java.UTXO pool,return true if transaction is successfully handled
     */
